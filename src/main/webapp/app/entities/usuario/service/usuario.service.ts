...
type RestOf<T extends IUsuario | NewUsuario> = Omit<T, 'dataCadastro' | 'ultimoAcesso'> & {
...
export class UsuarioService {
  ...
  protected convertDateFromClient<T extends IUsuario | NewUsuario | PartialUpdateUsuario>(usuario: T): RestOf<T> {
    return {
      ...usuario,
      dataCadastro: usuario.dataCadastro?.toJSON() ?? null,
      ultimoAcesso: usuario.ultimoAcesso?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restUsuario: RestUsuario): IUsuario {
    return {
      ...restUsuario,
      dataCadastro: restUsuario.dataCadastro ? dayjs(restUsuario.dataCadastro) : undefined,
      ultimoAcesso: restUsuario.ultimoAcesso ? dayjs(restUsuario.ultimoAcesso) : undefined,
    };
  }
