...
import { EntityArrayResponseType, TipoAtividadeService } from '../service/tipo-atividade.service';
...
export class TipoAtividadeComponent implements OnInit {
  ...
  trackId = (_index: number, item: ITipoAtividade): number => this.tipoAtividadeService.getTipoAtividadeIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.tipoAtividades = this.refineData(dataFromBody);
  }

  protected refineData(data: ITipoAtividade[]): ITipoAtividade[] {
    return data.sort(this.sortService.startSort(this.predicate, this.ascending ? 1 : -1));
  }

  protected fillComponentAttributesFromResponseBody(data: ITipoAtividade[] | null): ITipoAtividade[] {
    return data ?? [];
  }
  ...
  protected queryBackend(predicate?: string, ascending?: boolean): Observable<EntityArrayResponseType> {
    ...
    return this.tipoAtividadeService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
